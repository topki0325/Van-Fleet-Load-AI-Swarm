...
- ✅ 双语文档支持（中文 + 英语）
- ✅ **自定义 Relay 提供商支持**：可手动添加中转站 API 地址及自定义 Key Header/Prefix
- ✅ **AI 实体快速扩展（Burst Mode）**：支持 1-10 个实体一键克隆，实现单 API Key 的高并发满载
- ✅ **智能项目向导（多模式支持）**：
  - 集成 Windows 文件夹选择器（`rfd`），自动创建项目结构
  - **文章快速写**：三阶段自动化流程（生成目录 -> 分组并行写作 -> 主模型拼合全篇）
  - **网站原型快递搭建**：采用 HTML5/Tailwind/JS 技术栈，自动生成响应式页面原型
  - **软件原型快递搭建**：支持 Rust/Python，自动化架构设计与模块集成
  - **游戏原型快递搭建**：基于 Phaser.js 引擎，自动构建玩法循环与交互脚本
  - 自动创建本地文件结构（spec/src/index.html）并提交异步任务流到后端调度器
...
- 引入 `rfd` 依赖，解决原生 GUI 下的文件夹选择对话框需求
